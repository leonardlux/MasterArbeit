...
\begin{frame}
    \begin{block}{$Z$-Measurement Error}
        \begin{equation}
...
\begin{frame}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Oneshot (syndrome readout) \textcolor{red}{Explain this better}
            \item Fault tolerant by design 
            \item More (physical) qubits needed
            \item More than next neighbour interaction needed
        \end{itemize}
...
\begin{frame}{Threshold}
     \begin{itemize}
        ...
        \item find physical error rate under which we can arbitrary limit the logical error rate
        \item by increasing the code distance
        \item dependent on: encoding, error model, circuit layout
        \item How to find it 
        \item What is the optimal threshold
     \end{itemize}
\end{frame}

\section{Method}
\begin{frame}{Method}
    Final Goal: Determine the optimal threshold of Steane Type EC using an ML Decoder
    \begin{itemize}
        \item 
    \end{itemize}
\end{frame}
